...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        # Avergae doc length for BM25 calculation (relevant only for body index)
        self.avg_doc_len = 0
        # Documents length dictionary for BM25 calculation (relevant only for body index)
        self.doc_len_dict = defaultdict(list)
...
#                 tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
#                 posting_list.append((doc_id, tf))
            for i in range(0, len(b), 8):  # Adjust loop to process 8 bytes at a time
                doc_id = int.from_bytes(b[i:i+4], 'big')
                tf = int.from_bytes(b[i+4:i+8], 'big')
                posting_list.append((doc_id, tf))
        return posting_list
...
#                               for doc_id, tf in pl])
                b = b''.join([doc_id.to_bytes(4, 'big') + tf.to_bytes(4, 'big') for doc_id, tf in pl])
